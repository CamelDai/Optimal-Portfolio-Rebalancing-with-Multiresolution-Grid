...
def alpha_calibration(fund_index,Fund_mu,Fund_cov,saa_target):
    ...
    return np.round(alpha_list[np.argmin(error_test)],2)
        ...
        if i in be_bought:
            ...
            if fund_target_ratio[i]>weight[i]:
                while target_expand[i][-1]+inter<interval[i][1]:
                    target_expand[i].append(target_expand[i][-1]+inter)
                while target_expand[i][0]-inter>interval[i][0] or target_expand[i][0]>weight[i]:
                    target_expand[i].insert(0,target_expand[i][0]-inter)
            else:
                if fund_target_ratio[i]<weight[i]:
...
after_one_period_return=after_one_period_data[length_of_one_period-1]/after_one_period_data[0]

portfolio_growth=np.dot(fund_target_ratio,after_one_period_return)
weight=fund_target_ratio*after_one_period_return/portfolio_growth
period_weight[-1].append(weight.tolist())
#len(saa_target)

for period in range(1,len(saa_target)):
