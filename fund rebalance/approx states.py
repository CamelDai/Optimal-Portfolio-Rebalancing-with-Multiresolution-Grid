...
mu_annually=daily_returns.mean().values*252 # 年化期望報酬
std_annually=daily_returns.std().values*np.sqrt(252) # 年化波動度
cov_annually=daily_returns.cov().values*252 # 年化共變異數矩陣

# 一開始 growth 是由下面註解掉的城市算出來的，但因為重複使用性高，就將它存起來
